...
class LoggerAppenderMongoDB extends LoggerAppender {
	...
	public function activateOptions() {
		try {		
			if($this->userName!=""){
				$uri=sprintf('%s:%s@%s:%d/%s',$this->userName,$this->password ,$this->host, $this->port,$this->databaseName);
			}else{
				$uri=sprintf('%s:%d/%s', $this->host, $this->port,$this->databaseName);
			}
			if (!preg_match('/^mongodb\:\/\//', $uri))
			{
				$uri = self::DEFAULT_MONGO_URL_PREFIX . $uri;
			}
			$this->manager =new MongoDB\Driver\Manager($uri);	
		} catch (Exception $ex) {
			$this->closed = true;
			$this->warn('mongo database connect error:');
		}
	}
	...
	public function append(LoggerLoggingEvent $event)
	{
		try
		{
			if ($this->manager)
			{
				...
				$writeConcern = new MongoDB\Driver\WriteConcern(MongoDB\Driver\WriteConcern::MAJORITY, $this->timeout);
				$db_collection = sprintf('%s.%s', $this->databaseName, $this->collectionName);
				try
				{
					$result = $this->manager->executeBulkWrite($db_collection, $bulk, $writeConcern);
